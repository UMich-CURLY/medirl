...
for epoch in range(n_epoch):
    ...
    for index, (feat, robot_traj, human_past_traj, robot_past_traj, demo_rank) in enumerate(train_loader_robot):
        ...
        if step % vis_per_steps == 0 and not step ==0 :
            visualize_batch([robot_traj[0]], robot_traj, feat, r_var_r, values_list_r, svf_diff_var_r.float() , step, vis2, grid_size, train=True, policy_sample_list=sampled_trajs_r, rank_list=demo_rank)
            if step == 0:
                step += 1
                continue
